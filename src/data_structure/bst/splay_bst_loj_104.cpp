...
class task {
	...
	vector<query> qa;
	vector<int> ans;
	struct node : baseinfo<splay::node::type<node, int, 1>> {
		using base::base;
	};
	class tree : public
				baseinfos<
	...
	public:
		...
		node *kth(int k, int d) {
			auto ret = base::kth(root, k, d);
			base::select_by_size(root, k);
			return ret;
		}
		pair<int, node *> search(int k, bool d, bool contain) {
			auto ret = base::search(root, k, d, contain);
			base::select_by_key(root, k);
			return ret;
		}
	} dsm;
	void preprocess() {
